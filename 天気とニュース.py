...
def main():
    ...
    # タブの作成（ニュース・天気機能 と レシピ機能）
    tab1, tab2 = st.tabs(["🌤️ ニュースと天気", "🍳 冷蔵庫レシピメーカー"])
    ...
    with tab2:
        st.markdown('<div class="section-title">🍳 冷蔵庫の余り物でレシピ検索</div>', unsafe_allow_html=True)
        st.write("冷蔵庫にある食材を入力するだけで、人気レシピサイトでの検索結果にすぐアクセスできます。")
        
        ingredients = st.text_input("食材を入力してください（スペース区切り）", placeholder="例：豚肉 キャベツ もやし")
        
        if ingredients:
            st.markdown('<div class="section-title" style="font-size: 1.4rem; margin-top: 20px;">🔍 レシピを見る</div>', unsafe_allow_html=True)
            encoded_ing = quote(ingredients.replace(" ", " ").replace("　", " "))
            
            cookpad_url = f"https://cookpad.com/search/{encoded_ing}"
            kurashiru_url = f"https://www.kurashiru.com/search?query={encoded_ing}"
            delish_url = f"https://delishkitchen.tv/search?q={encoded_ing}"
            
            st.markdown(f"""
            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                <a href="{cookpad_url}" target="_blank" style="text-decoration: none; flex: 1;">
                    <div class="glass-card" style="text-align: center; border-bottom: 5px solid #cb2027;">
                        <span style="font-size: 1.5rem; font-weight: bold; color: #cb2027;">Cookpad</span><br>
                        <span style="font-size: 0.9rem; color: #555;">でレシピを開く</span>
                    </div>
                </a>
                <a href="{kurashiru_url}" target="_blank" style="text-decoration: none; flex: 1;">
                    <div class="glass-card" style="text-align: center; border-bottom: 5px solid #8bc34a;">
                        <span style="font-size: 1.5rem; font-weight: bold; color: #8bc34a;">クラシル</span><br>
                        <span style="font-size: 0.9rem; color: #555;">でレシピを開く</span>
                    </div>
                </a>
                <a href="{delish_url}" target="_blank" style="text-decoration: none; flex: 1;">
                    <div class="glass-card" style="text-align: center; border-bottom: 5px solid #f9a825;">
                        <span style="font-size: 1.5rem; font-weight: bold; color: #f9a825;">デリッシュキッチン</span><br>
                        <span style="font-size: 0.9rem; color: #555;">でレシピを開く</span>
                    </div>
                </a>
            </div>
